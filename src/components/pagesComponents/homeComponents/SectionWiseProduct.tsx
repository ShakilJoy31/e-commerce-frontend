...
export default function SectionWiseProduct({sectionWiseProducts, isLoading, companyData}) {
            ...
            sectionWiseProducts?.map(
              (section: any, sectionIdx: number) => (
                <div key={sectionIdx} className="">
                  ...
                  {/* Swiper for products in this section */}
                  <div className="relative mt-5">
                    <Swiper
                      ...
                      navigation={{
                        prevEl: `.prev-button-${sectionIdx}`,
                        nextEl: `.next-button-${sectionIdx}`,
                      }}
                      loop={true}
                      ...
                      {section?.products.map((item: any) => {
                        ...
                        return (
                          <SwiperSlide key={product.id}>
                            <div className="relative flex flex-col justify-between h-auto swiper-slide-cover border-[1px] rounded-md">
                              <ProductCard
                    ...
                    {/* Custom navigation buttons (must have unique classes) */}
                    <button
                      className={`prev-button-${sectionIdx} absolute left-14 top-[45%] z-30 -translate-x-full p-2 text-black bg-gray-200 rounded-full hover:bg-gray-300`}
                      aria-label="Previous Slide"
                    >
                      <IoIosArrowBack className="w-6 h-6" />
                    </button>
                    <button
                      className={`next-button-${sectionIdx} absolute right-14 top-[45%] z-30 translate-x-full p-2 text-black bg-gray-200 rounded-full hover:bg-gray-300`}
