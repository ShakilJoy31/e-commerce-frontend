...
import AddEditCategoryOfferModal from "./AddEditCategoryOfferModal";
import { 
  useAddCategoryOfferMutation, 
  useDeleteCategoryOfferMutation, 
  useGetCategoryOffersQuery, 
  useUpdateCategoryOfferMutation 
} from "@/components/store/api/categoryWiseOffer/categoryWiseOfferApi";
...
const CategoryWiseOfferList = () => {
  ...
  const [addOffer, { isLoading: addLoading, error: addError }] = useAddCategoryOfferMutation();
  const [updateOffer, { isLoading: editLoading, error: editError }] = useUpdateCategoryOfferMutation();
  const [deleteOffer, { isLoading: deleteLoading, error: deleteError }] = useDeleteCategoryOfferMutation();
  ...
  const handleSaveOffer = async (offerData: { categoryId: number; offerId: number }) => {
    try {
      if (currentOffer?.id) {
            ...
            renderRow={(row: any, index: number) => {
              ...
              return (
                <>
                  ...
                  <td className="px-4 py-2 font-medium">{row.category?.name}</td>
...
export default CategoryWiseOfferList;