...
import { useAddPopupMutation } from "@/components/store/api/popupList/popupApi";
...
export default function AddPopup() {
  ...
  const handleSaveCategory = async (data: any) => {
    ...
    try {
      ...
      await addCategory(updateData).unwrap();
      ...
      navigate("/kry-admin-portal/popup-list");
    } catch (error) {
      ...
      <div className="bg-white shadow  rounded-lg grid grid-cols-3 items-center gap-4 py-3 px-5">
        ...
        <div className="flex items-center gap-1 mt-1">
          <input
            ...
            className="w-5 h-5"
          />
          <label htmlFor="" className="-mt-1">isActive?</label>
        </div>
      </div>
