...
import { useAddTagMutation, useDeleteTagMutation, useGetTagsQuery, useUpdateTagMutation } from "@/components/store/api/tags/tagsApi";

const Tags = () => {
  const [searchTag, setSearchTag] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const { toast } = useToast();
  const [currentTag, setCurrentTag] = useState<{ id: number | null; name: string } | undefined>(undefined);
  ...
  const [addTag, { isLoading: addLoading, error: addError }] = useAddTagMutation();
  const [updateTag, { isLoading: editLoading, error: editError }] = useUpdateTagMutation();
  const [deleteTag, { isLoading: deleteLoading, error: deleteError }] = useDeleteTagMutation();
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      toast({
        title: "Success",
        description: "Selected tags have been deleted successfully",
      });
      setSelectedRows([]);
      refetch();
    } catch (err) {
  ...
  const handleSaveTag = async (id: number | null, name: string) => {
    try {
      ...
      if (result.success) {
        toast({
          title: id ? "Update Tag" : "Add Tag",
          description: toastMessageGenerator(id ? "update" : "add", "tag"),
        });
        setModalOpen(false);
        refetch();
      }
  ...
  const handleDeleteTag = async (id: number) => {
    try {
      ...
      toast({ title: "Tag deleted successfully", description: "The tag has been deleted." });
      refetch();
    } catch (err) {
      ...
      <div className="bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="font-bold w-4 h-4" /> Add Product Tag
            </button>
          </div>
        </div>
