...
export default function PaymentFailedMessage() {
  ...
  useEffect(() => {
    if (data?.data?.paymentFailedMessage) {
      setPaymentFailedMessage(data.data.paymentFailedMessage);
    }
  }, [data]);

  const handleUpdate = async () => {
    try {
      await updateField({ paymentFailedMessage }).unwrap();
      toast({
        title: "Success",
        description: "Payment failed message updated successfully",
      });
      setIsEditModalOpen(false);
    } catch (err) {
      console.error("Failed to update payment failed message:", err);
      toast({
        title: "Error",
        description: "Failed to update payment failed message",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg min-h-screen shadow p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-semibold mb-6">Payment Failed Message Template</h1>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            <tr>
              ...
              <td className="px-6 py-4 text-sm max-w-md truncate">
                {paymentFailedMessage || "No message set"}
              </td>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <div className="space-y-4">
            ...
            <textarea
              value={paymentFailedMessage}
              onChange={(e) => setPaymentFailedMessage(e.target.value)}
              rows={6}
