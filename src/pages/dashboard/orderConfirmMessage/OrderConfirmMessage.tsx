...
export default function OrderConfirmMessage() {
  ...
  useEffect(() => {
    if (data?.data?.orderConfirmMessage) {
      setOrderConfirmMessage(data.data.orderConfirmMessage);
    }
  }, [data]);

  const handleUpdate = async () => {
    try {
      await updateField({ orderConfirmMessage }).unwrap();
      toast({
        title: "Success",
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            <tr>
              ...
              <td className="px-6 py-4 text-sm max-w-md truncate">
                {orderConfirmMessage || "No message set"}
              </td>
