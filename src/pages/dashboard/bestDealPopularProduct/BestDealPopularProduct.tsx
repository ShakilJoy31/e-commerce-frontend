...
import { useAddPopularProductMutation, useDeletePopularProductMutation, useGetPopularProductsQuery, useUpdatePopularProductMutation } from "@/components/store/api/dealPopularProduct/dealPopularProductApi";
import AddEditPopularProductModal from "./AddEditPopularProductModal";
...
const BestDealPopularProduct = () => {
  ...
  const { data, refetch, isLoading } = useGetPopularProductsQuery({
  ...
  const handleDeleteProduct = async (id: number) => {
    ...
    toast({
      title: "Popular Product deleted successfully",
      description: "The product has been removed from the popular.",
    });
    refetch();
  };
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (modalType === "add") {
      ...
      if (result.data.success) {
        toast({
          title: "Add Best Popular Product Message",
      ...
      if (result.data.success) {
        toast({
          title: "Update Best Popular Product Message",
    ...
    <PageWrapper>
      <div className="bg-gray-100 min-h-screen p-4">
         <div className="flex justify-between items-center mb-5">
          <h1 className="text-2xl font-semibold">Best Popular Product</h1>
          <button
            className="flex items-center bg-blue-500 text-white px-4 py-1 rounded"
            onClick={handleAddProduct}
          >
            <Plus className="mr-2" /> Add Best Popular Product
          </button>
        </div>
        ...
        {deleteError && "data" in deleteError && (
          <Alert variant="destructive">
            ...
            <AlertTitle>Best Popular Product Error</AlertTitle>
            <AlertDescription>
...
export default BestDealPopularProduct;