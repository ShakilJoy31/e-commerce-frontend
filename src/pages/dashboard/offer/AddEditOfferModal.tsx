...
import { useCreateOfferMutation } from "@/components/store/api/products/offerApi";
...
export default function AddEditOfferModal() {
  ...
  const [addOffer, { isLoading: addLoading, error: err }] =
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await addOffer(cleanData).unwrap();
  ...
  return (
    <div>
      <div className="w-full lg:min-w-[1000px] overflow-hidden py-10 px-5 overflow-y-auto">
        <div>
          <div className="py-5">
            <Heading variant="primary">Create Offer</Heading>
          </div>
        </div>
        ...
        <div className="my-10 flex justify-end">
          <Button
            onClick={handleSave}
            disabled={addThumbnailLoading || addLoading}
          >
            {addThumbnailLoading || addLoading ? <ButtonLoader /> : "Save"}
          </Button>
        </div>
      </div>
    </div>
