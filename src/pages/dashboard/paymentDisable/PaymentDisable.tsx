...
const PaymentSettings = () => {
  ...
  const { control, handleSubmit, setValue } = useForm<PaymentSettingsFormData>({
    defaultValues: {
  ...
  useEffect(() => {
    if (companyData?.data?.length > 0) {
      ...
      setValue("isOnlinePayment", company.isOnlinePayment || false);
      setValue("isEmiPayment", company.isEmiPayment || false);
    }
  }, [companyData?.data, setValue]);

  const handleSavePaymentSettings = async (data: PaymentSettingsFormData) => {
    try {
      const payload = {
        id: companyData?.data[0]?.id,
        data: {
          isOnlinePayment: data.isOnlinePayment,
          isEmiPayment: data.isEmiPayment,
        }
      };
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500">
              Configure payment method availability
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Enable/disable online payment gateway
                </p>
              </div>
              <Controller
            ...
            <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Enable/disable EMI payment options
                </p>
              </div>
              <Controller
                name="isEmiPayment"
