...
const AddEditNewArrivalProductModal: React.FC<AddEditBestDealProductModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  ...
  const { data: products, isLoading:isProductLoading } = useGetSearchProductsQuery({ page: 1, size: 10000 });
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg p-6 shadow-lg w-1/3">
        <h2 className="text-lg font-semibold mb-4 pl-5">
          {initialData ? "Edit New Arrival Product" : "Add New Arrival Product"}
        </h2>
        <form onSubmit={handleSubmit(onSubmitForm)} className="px-5">
          ...
          {err && "data" in err && (
            <Alert variant="destructive">
              ...
              <AlertTitle>New Arrival product Error</AlertTitle>
              <AlertDescription>
                {(err.data as { message?: string })?.message || "Something went wrong! Please try again."}
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="flex justify-end gap-4">
            ...
            <button type="submit" className="px-4 flex items-center gap-1 py-2 rounded bg-blue-500 text-white">
...
export default AddEditNewArrivalProductModal;