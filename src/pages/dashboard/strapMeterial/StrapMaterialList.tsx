...
import { useAddMaterialMutation, useDeleteMaterialMutation, useGetMaterialsQuery, useUpdateMaterialMutation } from "@/components/store/api/mateiral/materialApi";
...
const MaterialList = () => {
  ...
  const [addMaterial, {isLoading:addLoading, error:addError}] = useAddMaterialMutation();
  const [updateMaterial, {isLoading:editLoading, error:editError}] = useUpdateMaterialMutation();
  ...
  const handleSaveMaterial = async (
    ...
    try {
      if (id) {
        ...
        if (result.success) {
          toast({
            title: "Update Material Message",
            description: toastMessageGenerator("update", "material"),
          });
          setModalOpen(false);
        }
      } else {
        ...
        if (result.success) {
          toast({
            title: "Add Material Message",
            description: toastMessageGenerator("add", "material"),
          });
          setModalOpen(false);
        }
      }
      ...
      <div className="bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="font-bold w-4 h-4" /> Add Material
            </button>
          </div>
        </div>
