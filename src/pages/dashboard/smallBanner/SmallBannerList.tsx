...
} from "@/components/store/api/smallBanner/smallBannerApi";
import Table from "@/components/ui/table";
import { AlertCircle, Plus } from "lucide-react";
import { useEffect, useState } from "react";
import { FiEdit, FiSearch, FiTrash2 } from "react-icons/fi";
import AddEditSmallBannerModal from "./AddEditSmallBannerModal";
...
const SmallBannerList = () => {
  ...
  const [currentBanner, setCurrentBanner] = useState<
    { bannerId: number; image: string } | undefined
  >(undefined);
  ...
  // API Calls
  const { data, isLoading, isError, refetch } = useGetSmallBannersQuery({
...
const handleSaveBanner = async (
  ...
  const handleEditBanner = (banner: { id: number; image: string }) => {
    setCurrentBanner({
      bannerId: banner.id,
      image: banner.image,
    });
    ...
    <PageWrapper>
      <div className="bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-5">
          <h1 className="text-2xl font-semibold">Side Slider</h1>
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="font-bold w-4 h-4" /> Add Side Slider
            </button>
          </div>
        </div>
        ...
        {deleteError && "data" in deleteError && (
          <Alert variant="destructive">
            ...
            <AlertTitle>Small Banner Error</AlertTitle>
            <AlertDescription>
