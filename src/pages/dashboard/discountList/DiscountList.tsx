...
import { useAddDiscountMutation, useDeleteDiscountMutation, useGetDiscountInfoAllQuery, useUpdateDiscountMutation } from "@/components/store/api/discountApi/discountApi";
...
const DiscountList = () => {
  ...
  const [currentDiscount, setCurrentDiscount] = useState<
    { id: number | null; code: string; discountType:"FIXED" | "PERCENTAGE"; discount:number; expireDate:string } | undefined
  >(undefined);
  ...
  const handleSaveDiscount = async (id: number | null, code: string, discountType: "FIXED" | "PERCENTAGE", discount: number, expireDate: string) => {
    try {
      if (id) {
        // Edit Discount
        ...
        if (result.success) {
          toast({
            title: "Update Discount Message",
            description: toastMessageGenerator("update", "discount"),
          });
        ...
        if (result.success) {
          toast({
            title: "Add Discount Message",
            description: toastMessageGenerator("add", "discount"),
          });
          setModalOpen(false); // Close the modal
        }
      }
          ...
          <Table
            ...
            onSelectAll={() => {
              if (selectedRows.length === discounts.length) {
                ...
                setSelectedRows(discounts.map((discount) => discount.id));
              }
            }}
