...
import AddEditCustomMessageModal from "./AddEditCustomMessageModal";
import { useAddCustomMessageMutation, useDeleteCustomMessageMutation, useGetCustomMessageQuery, useUpdateCustomMessageMutation } from "@/components/store/api/customMessage/customMessage";
...
const CustomMessage = () => {
  ...
  const [currentTag, setCurrentTag] = useState<{ id: number | null; customMessage: string } | undefined>(undefined);
  ...
  const { data, isLoading, isError, refetch } = useGetCustomMessageQuery({
    page: pagination.page,
    size: pagination.size,
    search: searchTag,
  });

  const [addTag, { isLoading: addLoading, error: addError }] = useAddCustomMessageMutation();
  const [updateTag, { isLoading: editLoading, error: editError }] = useUpdateCustomMessageMutation();
  const [deleteTag, { isLoading: deleteLoading, error: deleteError }] = useDeleteCustomMessageMutation();
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      toast({
        title: "Success",
        description: "Selected custom message have been deleted successfully",
      });
      setSelectedRows([]);
      refetch();
    } catch (err) {
      ...
      toast({
        title: "Error",
        description: "Failed to delete selected custom message",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSaveTag = async (id: number | null, name: string) => {
    try {
      ...
      if (result.success) {
        toast({
          title: id ? "Update Custom Message" : "Add Custom Message",
          description: toastMessageGenerator(id ? "update" : "add", "custom message"),
        });
        setModalOpen(false);
        refetch();
      }
  ...
  const handleDeleteTag = async (id: number) => {
    try {
      ...
      toast({ title: "Custom Message deleted successfully", description: "The custom message has been deleted." });
      refetch();
    } catch (err) {
    ...
    <PageWrapper>
      <div className="bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-5">
          <h1 className="text-2xl font-semibold">Custom Message</h1>
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="font-bold w-4 h-4" /> Add Custom Message
            </button>
          </div>
        </div>
        ...
        {isError && <div className="text-center py-6 text-red-500">Failed to fetch custom message.</div>}

        {!isLoading && !isError && (
          <Table
            headers={["SL", "Custom Message", "Actions"]}
            data={tags}
            ...
            renderRow={(row, index: number) => {
              ...
              return (
                <>
                  ...
                  <td className="px-4 py-2 font-medium">{row.customMessage}</td>
