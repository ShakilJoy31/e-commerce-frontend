...
import AddEditCourierModal from "./AddEditCourierModal";
import { useAddCourierMutation, useDeleteCourierMutation, useGetTCourierQuery, useUpdateCourierMutation } from "@/components/store/api/courier/courierApi";
...
const AddCourier = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetTCourierQuery({
    page: pagination.page,
    size: pagination.size,
    search: searchTag,
  });

  const [addTag, { isLoading: addLoading, error: addError }] = useAddCourierMutation();
  const [updateTag, { isLoading: editLoading, error: editError }] = useUpdateCourierMutation();
  const [deleteTag, { isLoading: deleteLoading, error: deleteError }] = useDeleteCourierMutation();
      ...
      <div className="bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="font-bold w-4 h-4" /> Add Courier
            </button>
          </div>
        </div>
