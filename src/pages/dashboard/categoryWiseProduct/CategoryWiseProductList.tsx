...
} from "@/components/store/api/categoryWiseProduct/categoryWiseProductApi";
import AddEditCategoryWiseProductModal from "./AddEditCategoryWiseProductModal";
...
const headers = ["SL", "Product", "Category", "Action"];
...
const CategoryWiseProduct = () => {
  ...
  const { data, refetch, isLoading } = useGetCategoryWiseProductsQuery({
  ...
  const [addCategoryWiseProduct, { isLoading: addLoading, error: addError }] =
    useAddCategoryWiseProductMutation();
  const [
    updateCategoryWiseProduct,
    { isLoading: editLoading, error: editError },
  ] = useUpdateCategoryWiseProductMutation();
  const [
    deleteCategoryWiseProduct,
  ...
  const handleDeleteProduct = async (id: number) => {
    await deleteCategoryWiseProduct(id);
    toast({
      title: "Category-wise Product deleted successfully",
      description: "The product has been removed from the category.",
    });
    refetch();
  };
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (modalType === "add") {
      const result = await addCategoryWiseProduct(formattedData);
      if (result.data.success) {
        toast({
          title: "Add Category Product Message",
          description: toastMessageGenerator("add", "product"),
        });
        setIsModalOpen(false);
      }
    } else if (modalType === "edit") {
      const result = await updateCategoryWiseProduct({
        id: selectedProduct!.id,
        data: formattedData,
      });
      if (result.data.success) {
        toast({
          title: "Update Category Product Message",
  ...
  const handleSelectAll = () => {
    if (selectedRows.length === categoryWiseProducts.length) {
      setSelectedRows([]); // Deselect all
    } else {
      setSelectedRows(categoryWiseProducts.map((row) => row.id));
    }
  };
  if (isLoading) {
    ...
    <PageWrapper>
      <div className="bg-gray-100 min-h-screen p-4">
        <div className="flex justify-between items-center mb-5">
          <h1 className="text-2xl font-semibold">Category Wise Product</h1>
          <button
            className="flex items-center bg-blue-500 text-white px-4 py-1 rounded"
            onClick={handleAddProduct}
          >
            <Plus className="mr-2" /> Add Category Wise Product
          </button>
        </div>
        ...
        {categoryWiseProducts.length > 0 ? (
          ...
          <p className="text-center text-gray-500">
            No Category wise products found.
          </p>
        )}
        ...
        {isModalOpen && (
          <AddEditCategoryWiseProductModal
            isOpen={isModalOpen}
