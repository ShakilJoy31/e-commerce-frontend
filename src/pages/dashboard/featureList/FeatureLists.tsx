...
import AddEditFeatureListModal from "./AddEditFeatureListModal";
import { useAddFeaturesMutation, useDeleteFeaturesMutation, useGetFeaturesQuery, useUpdateFeaturesMutation } from "@/components/store/api/featurelist/featurelistApi";
...
const FeatureLists = () => {
  ...
  // API Calls
  const { data, isLoading, isError, refetch } = useGetFeaturesQuery({
  ...
  const handleSaveFeature = async (id: number | null, name: string) => {
    try {
      if (id) {
        ...
        if (result.success) {
          toast({
            title: "Update Featur Message",
            description: toastMessageGenerator("update", "feature"),
          });
          setModalOpen(false);
        }
      } else {
        ...
        if (result.success) {
          toast({
            title: "Add Feature Message",
            description: toastMessageGenerator("add", "feature"),
          });
          setModalOpen(false);
        }
      }
  ...
  const handleDeleteFeature = async (id: number) => {
    try {
      ...
      toast({ title: "Feature deleted successfully!", variant: "default" });
      refetch();
    } catch (err) {
      toast({ title: "Error deleting feature.", variant: "destructive" });
      ...
      <div className="bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="font-bold w-4 h-4" /> Add Feature
            </button>
          </div>
        </div>
