...
export default function OrderCreationMessage() {
  ...
  useEffect(() => {
    if (data?.data?.orderCreationMessage) {
      setOrderCreationMessage(data.data.orderCreationMessage);
    }
  }, [data]);

  const handleUpdate = async () => {
    try {
      await updateField({ orderCreationMessage }).unwrap();
      toast({
        title: "Success",
        description: "Order creation message updated successfully",
      });
      setIsEditModalOpen(false);
    } catch (err) {
      console.error("Failed to update Order creation message:", err);
      toast({
        title: "Error",
        description: "Failed to update Order creation message",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg min-h-screen shadow p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-semibold mb-6">Order Creation Message Template</h1>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            <tr>
              ...
              <td className="px-6 py-4 text-sm max-w-md truncate">
                {orderCreationMessage || "No message set"}
              </td>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <div className="space-y-4">
            <h2 className="text-xl font-bold">Edit Order Creation Message</h2>
            <textarea
              value={orderCreationMessage}
              onChange={(e) => setOrderCreationMessage(e.target.value)}
              rows={6}
