...
} from "@/components/store/api/blogTags/blogTagsApi";
...
const BlogTags = () => {
  ...
  const [addTag, { isLoading: addLoading, error: addError }] = useAddBlogTagMutation();
  const [updateTag, { isLoading: editLoading, error: editError }] = useUpdateBlogTagMutation();
  ...
  const handleSaveTag = async (id: number | null, name: string) => {
    try {
      ...
      if (result.success) {
        toast({
          title: id ? "Update Blog Tag" : "Add Blog Tag",
