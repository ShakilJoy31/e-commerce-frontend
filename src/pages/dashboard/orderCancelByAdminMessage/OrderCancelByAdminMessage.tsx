...
export default function OrderCancelByAdminMessage() {
  ...
  const [orderCancelByAdminMessage, setOrderCancelByAdminMessage] = useState("");
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);

  useEffect(() => {
    if (data?.data?.orderCancelByAdminMessage) {
      setOrderCancelByAdminMessage(data.data.orderCancelByAdminMessage);
    }
  }, [data]);

  const handleUpdate = async () => {
    try {
      await updateField({ orderCancelByAdminMessage }).unwrap();
      toast({
        title: "Success",
        description: "Order cancel message updated successfully",
      });
      setIsEditModalOpen(false);
    } catch (err) {
      console.error("Failed to update Order cancel message message:", err);
      toast({
        title: "Error",
        description: "Failed to update Order cancel confirm message",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg min-h-screen shadow p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-semibold mb-6">Order Cancel Message Template</h1>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            <tr>
              ...
              <td className="px-6 py-4 text-sm max-w-md truncate">
                {orderCancelByAdminMessage || "No message set"}
              </td>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <div className="space-y-4">
            <h2 className="text-xl font-bold">Edit Order Cancel Message</h2>
            <textarea
              value={orderCancelByAdminMessage}
              onChange={(e) => setOrderCancelByAdminMessage(e.target.value)}
              rows={6}
