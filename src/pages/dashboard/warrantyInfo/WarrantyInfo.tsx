...
import AddEditWarrantyInfoModal from "./AddEditWarrantyInfoModal";
import { useAddWarrantyInfoMutation, useDeleteWarrantyInfoMutation, useGetWarrantyInfosQuery, useUpdateWarrantyInfoMutation } from "@/components/store/api/warrantyInfo/warrantyInfoApi";

const WarrantyInfo = () => {
  const [searchRegularWarranty, setSearchRegularWarranty] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const { toast } = useToast();
  const [currentRegularWarranty, setCurrentRegularWarranty] = useState<{ id: number | null; name: string } | undefined>(undefined);
  ...
  const { data, isLoading, isError, refetch } = useGetWarrantyInfosQuery({
    page: pagination.page,
    size: pagination.size,
    search: searchRegularWarranty,
  });

  const [addRegularWarranty, { isLoading: addLoading, error: addError }] = useAddWarrantyInfoMutation();
  const [updateRegularWarranty, { isLoading: editLoading, error: editError }] = useUpdateWarrantyInfoMutation();
  const [deleteRegularWarranty, { isLoading: deleteLoading, error: deleteError }] = useDeleteWarrantyInfoMutation();
  ...
  const handleSaveRegularWarranty = async (id: number | null, name: string) => {
    try {
      const result = id
        ? await updateRegularWarranty({ id, name }).unwrap()
        : await addRegularWarranty({ name }).unwrap();

      if (result.success) {
        toast({
          title: id ? "Update Regular Warranty" : "Add Warranty Info",
          description: toastMessageGenerator(id ? "update" : "add", "Warranty Info"),
        });
        setModalOpen(false);
        refetch();
      }
  ...
  const handleDeleteRegularWarranty = async (id: number) => {
    try {
      ...
      toast({ 
        title: "Warranty Info deleted successfully", 
    ...
    <PageWrapper>
      <div className="bg-gray-100 min-h-screen">
        <div className="flex justify-between items-center mb-5">
          <h1 className="text-2xl font-semibold">Warranty Info</h1>
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="font-bold w-4 h-4" /> Add Warranty Info
            </button>
          </div>
        </div>
        ...
        {isError && (
          <div className="text-center py-6 text-red-500">
            Failed to fetch Warranties Info.
          </div>
        )}

        {!isLoading && !isError && (
          <Table
            headers={["SL", "Warranty Info", "Actions"]}
