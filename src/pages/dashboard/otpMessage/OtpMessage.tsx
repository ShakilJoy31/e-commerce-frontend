...
export default function OtpMessage() {
  ...
  useEffect(() => {
    if (data?.data?.otpMessage) {
      setOtpMessage(data.data.otpMessage);
    }
  }, [data]);

  const handleUpdate = async () => {
    try {
      await updateField({ otpMessage }).unwrap();
      toast({
        title: "Success",
        description: "OTP message updated successfully",
      });
      setIsEditModalOpen(false);
    } catch (err) {
      console.error("Failed to update OTP message:", err);
      toast({
        title: "Error",
        description: "Failed to update OTP message",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg min-h-screen shadow p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-semibold mb-6">OTP Message Template</h1>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <div className="space-y-4">
            <h2 className="text-xl font-bold">Edit OTP Message</h2>
            <textarea
              value={otpMessage}
