...
import { useGetProductsFeatureWiseQuery } from "@/components/store/api/products/productApi";
...
const FeatureWiseProduct = () => {
  ...
  const { data: featureWiseProducts, isLoading: ProductLoading } =
  ...
  useEffect(() => {
    if (featureWiseProducts) {
      setPagination((prev) => ({
        ...prev,
        page: (featureWiseProducts.meta.page ?? 0) + 1, // ✅ Ensure page is 1-based
        size: featureWiseProducts.meta.size, // ✅ Keep API-provided size
        meta: {
          page: (featureWiseProducts.meta.page ?? 0) + 1, // ✅ Ensure 1-based
          size: featureWiseProducts.meta.size,
          total: featureWiseProducts.meta.total,
          totalPage: featureWiseProducts.meta.totalPage || 1, // ✅ Ensure there's at least 1 page
          maxPrice: featureWiseProducts.meta.maxPrice,
          minPrice: featureWiseProducts.meta.minPrice,
        },
      }));
    }
  }, [featureWiseProducts]);
  ...
  if (!featureWiseProducts || featureWiseProducts.data.length === 0) {
        ...
        <div className="w-full lg:w-[85%] p-2 lg:p-4 border-none lg:border-l">
          <div className="flex items-center justify-between mt-5 bg-gray-100 p-4 rounded-md shadow-md">
            <p className="text-xl font-semibold text-primary">
              {featureWiseProducts.data[0]?.brand?.brand}
            </p>
            <p>
          ...
          {/* Product Grid */}
          {featureWiseProducts?.data.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5 my-10">
              {featureWiseProducts?.data?.map((product: any) => {
...
export default FeatureWiseProduct;
