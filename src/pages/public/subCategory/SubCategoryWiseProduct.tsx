...
import { useGetProductsSubCategoryWiseQuery } from "@/components/store/api/products/productApi";
...
const SubCategoryWiseProduct = () => {
  ...
  const { data: subCategoryWiseProducts, isLoading: ProductLoading } =
  ...
  const [filteredProducts, setFilteredProducts] = useState<any>(
    subCategoryWiseProducts?.data
  );

  useEffect(() => {
    if (subCategoryWiseProducts) {
      setPagination((prev) => ({
        ...prev,
        page: (subCategoryWiseProducts.meta.page ?? 0) + 1, // ✅ Ensure page is 1-based
        size: subCategoryWiseProducts.meta.size, // ✅ Keep API-provided size
        meta: {
          page: (subCategoryWiseProducts.meta.page ?? 0) + 1, // ✅ Ensure 1-based
          size: subCategoryWiseProducts.meta.size,
          total: subCategoryWiseProducts.meta.total,
          totalPage: subCategoryWiseProducts.meta.totalPage || 1, // ✅ Ensure there's at least 1 page
          maxPrice: subCategoryWiseProducts.meta.maxPrice,
          minPrice: subCategoryWiseProducts.meta.minPrice,
        },
      }));
      setMinPrice(subCategoryWiseProducts.meta.minPrice || 0);
      setMaxPrice(subCategoryWiseProducts.meta.maxPrice || 100000);
      setPriceRange([
        subCategoryWiseProducts.meta.minPrice || 0,
        subCategoryWiseProducts.meta.maxPrice || 100000,
      ]);
    }
  ...
  useEffect(() => {
    if (isFilterApplied) {
      ...
      setFilteredProducts(subCategoryWiseProducts?.data);
    }
  }, [
    fetchFilteredProducts,
    isFilterApplied,
  ...
  if (!subCategoryWiseProducts || subCategoryWiseProducts.data.length === 0) {
    return (
        ...
        <div className="w-full lg:w-[85%] p-2 lg:p-4 border-none lg:border-l">
          <div className="flex items-center justify-between mt-5 bg-gray-100 p-4 rounded-md shadow-md">
            <p className="text-xl font-semibold text-primary">
              {subCategoryWiseProducts.data[0]?.brand?.brand}
            </p>
            <p>
